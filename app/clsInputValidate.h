...
public:
	...
	{
		T Number = clsInputValidate::ReadNumber<T>();
	...
	/*static int ReadNumberBetween<int>(string ErrorMessage = "Invalid Number, Enter again\n")
{
	int Number;
	while (!(cin >> Number)) {
		cin.clear();
	...
	//static bool IsNumberBetween(T Number, double From, double To)
	//{
	//	if (Number >= From && Number <= To)
	//		return true;
	//	else
	...
	//static int ReadNumberBetween<int>Between(int From, int To, string ErrorMessage = "Number is not within range, Enter again:/\n")
	//{
